/*
 * Usare questo file al posto di stm32h7xx_hal_mmc.c perche'
 * aggiunge le operazioni necessarie ad accedere al RPMB
  */
...
// Aggiungo queste
#define MMC_CONTEXT_WRITE_MULTIPLE       ( (uint32_t) 0x00000100 )
#define MMC_CONTEXT_READ_MULTIPLE        ( (uint32_t) 0x00000200 )
...
static uint32_t MMC_SendStatus(
    MMC_HandleTypeDef * hmmc,
    uint32_t * pCardStatus) ;
static void MMC_PowerOFF(MMC_HandleTypeDef * hmmc) ;
static void MMC_Write_IT(MMC_HandleTypeDef * hmmc) ;
static void MMC_Read_IT(MMC_HandleTypeDef * hmmc) ;
static uint32_t MMC_HighSpeed(
    MMC_HandleTypeDef * hmmc,
    FunctionalState state) ;
static uint32_t MMC_DDR_Mode(
    MMC_HandleTypeDef * hmmc,
    FunctionalState state) ;
static HAL_StatusTypeDef MMC_ReadExtCSD(
    MMC_HandleTypeDef * hmmc,
    uint32_t * pFieldData,
    uint16_t FieldIndex,
    uint32_t Timeout) ;
static uint32_t MMC_PwrClassUpdate(
    MMC_HandleTypeDef * hmmc,
    uint32_t Wide,
    uint32_t Speed) ;
...
HAL_StatusTypeDef HAL_MMC_Init(MMC_HandleTypeDef * hmmc)
{
    ...
    if ( hmmc->State == HAL_MMC_STATE_RESET ) {
        ...
        hmmc->Read_DMADblBuf0CpltCallback =
            HAL_MMCEx_Read_DMADoubleBuf0CpltCallback ;
        hmmc->Read_DMADblBuf1CpltCallback =
            HAL_MMCEx_Read_DMADoubleBuf1CpltCallback ;
        hmmc->Write_DMADblBuf0CpltCallback =
    ...
    if ( hmmc->Init.BusWide != SDMMC_BUS_WIDE_1B ) {
        if ( HAL_MMC_ConfigWideBusOperation(hmmc,
                                            hmmc->Init.BusWide) != HAL_OK ) {
            return HAL_ERROR ;
        }
    }
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_ReadBlocks(
    MMC_HandleTypeDef * hmmc,
    uint8_t * pData,
    uint32_t BlockAdd,
    ...
    uint32_t Timeout)
{
    ...
    if ( hmmc->State == HAL_MMC_STATE_READY ) {
        ...
        if ( ( (hmmc->Ext_CSD[(MMC_EXT_CSD_DATA_SEC_SIZE_INDEX
                               / 4)] >> MMC_EXT_CSD_DATA_SEC_SIZE_POS)
               & 0x000000FFU ) != 0x0U ) {
        ...
        while ( !__HAL_MMC_GET_FLAG(hmmc,
                                    SDMMC_FLAG_RXOVERR | SDMMC_FLAG_DCRCFAIL
                                    | SDMMC_FLAG_DTIMEOUT
                                    | SDMMC_FLAG_DATAEND) ) {
            if ( __HAL_MMC_GET_FLAG(hmmc,
                                    SDMMC_FLAG_RXFIFOHF) &&
                 (dataremaining >= 32U) ) {
    ...
    uint32_t Timeout)
{
    ...
    if ( hmmc->State == HAL_MMC_STATE_READY ) {
        ...
        if ( ( (hmmc->Ext_CSD[(MMC_EXT_CSD_DATA_SEC_SIZE_INDEX
                               / 4)]
                >> MMC_EXT_CSD_DATA_SEC_SIZE_POS) & 0x000000FFU ) != 0x0U ) {
            if ( (NumberOfBlocks % 8U) != 0U ) {
                /* The number of blocks should be a multiple of 8 sectors of 512 bytes = 4 KBytes */
                hmmc->ErrorCode |= HAL_MMC_ERROR_BLOCK_LEN_ERR ;
                return HAL_ERROR ;
        ...
        dataremaining = config.DataLength ;
        while ( !__HAL_MMC_GET_FLAG(hmmc,
                                    SDMMC_FLAG_TXUNDERR | SDMMC_FLAG_DCRCFAIL
                                    | SDMMC_FLAG_DTIMEOUT
                                    | SDMMC_FLAG_DATAEND) ) {
            if ( __HAL_MMC_GET_FLAG(hmmc,
                                    SDMMC_FLAG_TXFIFOHE) &&
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_ReadBlocks_IT(
    MMC_HandleTypeDef * hmmc,
    uint8_t * pData,
    uint32_t BlockAdd,
    uint32_t NumberOfBlocks)
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_ReadBlocks_DMA(
    MMC_HandleTypeDef * hmmc,
    uint8_t * pData,
    uint32_t BlockAdd,
    uint32_t NumberOfBlocks)
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_Erase(
    MMC_HandleTypeDef * hmmc,
    ...
    uint32_t BlockEndAdd)
{
    ...
    if ( hmmc->State == HAL_MMC_STATE_READY ) {
        ...
        if ( (SDMMC_GetResponse(hmmc->Instance,
...
void HAL_MMC_IRQHandler(MMC_HandleTypeDef * hmmc)
{
    ...
    if ( (__HAL_MMC_GET_FLAG(hmmc,
        ...
        __HAL_MMC_DISABLE_IT(
            hmmc,
            SDMMC_IT_DATAEND | SDMMC_IT_DCRCFAIL | SDMMC_IT_DTIMEOUT   \
        ...
        if ( (context & MMC_CONTEXT_DMA) != 0U ) {
            ...
            if ( ( (context & MMC_CONTEXT_READ_MULTIPLE) != 0U ) ||
                 ( (context & MMC_CONTEXT_WRITE_MULTIPLE) != 0U ) ) {
            ...
            if ( ( (context & MMC_CONTEXT_READ_SINGLE_BLOCK) != 0U ) ||
                 ...
                 || ( (context & MMC_CONTEXT_READ_MULTIPLE) != 0U ) ) {
#if defined (USE_HAL_MMC_REGISTER_CALLBACKS) && \
                (USE_HAL_MMC_REGISTER_CALLBACKS == 1U)
        ...
        __HAL_MMC_DISABLE_IT(
            hmmc,
            SDMMC_IT_DATAEND | SDMMC_IT_DCRCFAIL
            | SDMMC_IT_DTIMEOUT   \
            | SDMMC_IT_TXUNDERR | SDMMC_IT_RXOVERR) ;
...
/**
  ...
  * @retval status
  */
HAL_StatusTypeDef HAL_MMC_RegisterCallback(
    MMC_HandleTypeDef * hmmc,
...
/**
  ...
  * @retval status
  */
HAL_StatusTypeDef HAL_MMC_UnRegisterCallback(
    MMC_HandleTypeDef * hmmc,
    HAL_MMC_CallbackIDTypeDef CallbackId)
{
    HAL_StatusTypeDef status = HAL_OK ;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_GetCardCID(
    MMC_HandleTypeDef * hmmc,
    HAL_MMC_CardCIDTypeDef * pCID)
{
    ...
    pCID->ProdName1 =
        ( ( (hmmc->CID[0]
             & 0x000000FFU) << 24U ) | ( (hmmc->CID[1] & 0xFFFFFF00U) >> 8U ) ) ;
    ...
    pCID->ProdSN =
        ( ( (hmmc->CID[2]
             & 0x00FFFFFFU) << 8U ) | ( (hmmc->CID[3] & 0xFF000000U) >> 24U ) ) ;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_GetCardCSD(
    MMC_HandleTypeDef * hmmc,
    HAL_MMC_CardCSDTypeDef * pCSD)
{
    ...
    if ( hmmc->MmcCard.CardType == MMC_LOW_CAPACITY_CARD ) {
        pCSD->DeviceSize =
            ( ( (hmmc->CSD[1]
                 & 0x000003FFU) << 2U ) | ( (hmmc->CSD[2] & 0xC0000000U) >> 30U ) ) ;

        pCSD->MaxRdCurrentVDDMin =
            (uint8_t) ( (hmmc->CSD[2] & 0x38000000U) >> 27U ) ;

        pCSD->MaxRdCurrentVDDMax =
            (uint8_t) ( (hmmc->CSD[2] & 0x07000000U) >> 24U ) ;

        pCSD->MaxWrCurrentVDDMin =
            (uint8_t) ( (hmmc->CSD[2] & 0x00E00000U) >> 21U ) ;

        pCSD->MaxWrCurrentVDDMax =
            (uint8_t) ( (hmmc->CSD[2] & 0x001C0000U) >> 18U ) ;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_GetCardInfo(
    MMC_HandleTypeDef * hmmc,
    ...
    uint32_t WideMode)
{
    ...
    if ( (hmmc->Instance->CLKCR & SDMMC_CLKCR_BUSSPEED) != 0U ) {
        if ( (hmmc->Instance->CLKCR & SDMMC_CLKCR_DDR) != 0U ) {
            errorstate = MMC_PwrClassUpdate(hmmc,
                                            WideMode,
...
HAL_StatusTypeDef HAL_MMC_Abort(MMC_HandleTypeDef * hmmc)
{
    ...
    if ( (CardState == HAL_MMC_CARD_RECEIVING) ||
         (CardState == HAL_MMC_CARD_SENDING) ) {
        hmmc->ErrorCode = SDMMC_CmdStopTransfer(hmmc->Instance) ;
    }
    if ( hmmc->ErrorCode != HAL_MMC_ERROR_NONE ) {
        return HAL_ERROR ;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_EraseSequence(
    MMC_HandleTypeDef * hmmc,
    uint32_t EraseType,
    uint32_t BlockStartAdd,
    uint32_t BlockEndAdd)
            ...
            if ( errorstate == HAL_MMC_ERROR_NONE ) {
                ...
                if ( errorstate == HAL_MMC_ERROR_NONE ) {
                    if ( (EraseType == HAL_MMC_SECURE_ERASE) ||
                         (EraseType == HAL_MMC_SECURE_TRIM_STEP2) ) {
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_ConfigSecRemovalType(
    MMC_HandleTypeDef * hmmc,
    uint32_t SRTMode)
{
    uint32_t srt ;
    ...
    if ( hmmc->State == HAL_MMC_STATE_READY ) {
        ...
        if ( HAL_MMC_GetSupportedSecRemovalType(hmmc, &srt) == HAL_OK ) {
            ...
            if ( (SRTMode & srt) != 0U ) {
                ...
                errorstate =
                    SDMMC_CmdSwitch( hmmc->Instance, ( 0x03100000U | (srt << 8U) ) ) ;
                if ( errorstate == HAL_MMC_ERROR_NONE ) {
                    /* While card is not ready for data and trial number for sending CMD13 is not exceeded */
                    count = SDMMC_MAX_TRIAL ;
                    do {
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_GetSupportedSecRemovalType(
    MMC_HandleTypeDef * hmmc,
            ...
            if ( count == 0U ) {
                ...
                if ( (response & 0x80U) != 0U ) {
                    ...
                    if ( errorstate == HAL_MMC_ERROR_NONE ) {
                        ...
                        sleep_timeout =
                            ( (hmmc->Ext_CSD[(
                                                 MMC_EXT_CSD_SLEEP_NOTIFICATION_TIME_INDEX
                                                 / 4)]
                               >>
                               MMC_EXT_CSD_SLEEP_NOTIFICATION_TIME_POS)
                              & 0x000000FFU ) ;
...
HAL_StatusTypeDef HAL_MMC_AwakeDevice(MMC_HandleTypeDef * hmmc)
{
    ...
    if ( hmmc->State == HAL_MMC_STATE_READY ) {
        ...
        /* Field S_A_TIEMOUT [217] */
        sleep_timeout =
            ( (hmmc->Ext_CSD[(MMC_EXT_CSD_S_A_TIMEOUT_INDEX
                              / 4)] >> MMC_EXT_CSD_S_A_TIMEOUT_POS)
              & 0x000000FFU ) ;
        ...
        errorstate =
            SDMMC_CmdSleepMmc( hmmc->Instance, (hmmc->MmcCard.RelCardAdd << 16U) ) ;
        if ( errorstate == HAL_MMC_ERROR_NONE ) {
            /* Wait that the device is ready by checking the D0 line */
            while ( ( !__HAL_MMC_GET_FLAG(hmmc,
                                          SDMMC_FLAG_BUSYD0END) ) &&
            ...
            if ( errorstate == HAL_MMC_ERROR_NONE ) {
                if ( HAL_MMC_GetCardState(hmmc) == HAL_MMC_CARD_STANDBY ) {
                    ...
                    errorstate =
                        SDMMC_CmdSelDesel( hmmc->Instance,
                                           (hmmc->MmcCard.RelCardAdd << 16U) ) ;
                    if ( errorstate == HAL_MMC_ERROR_NONE ) {
                        if ( HAL_MMC_GetCardState(hmmc) ==
                             HAL_MMC_CARD_TRANSFER ) {
                            /* Set the power-off notification to powered-on : Ext_CSD[34] = 1 */
                            errorstate =
                                SDMMC_CmdSwitch( hmmc->Instance, (0x03220100U) ) ;
                            if ( errorstate == HAL_MMC_ERROR_NONE ) {
                                /* While card is not ready for data and trial number for sending CMD13 is not exceeded */
                                count = SDMMC_MAX_TRIAL ;
                                do {
...
static uint32_t MMC_InitCard(MMC_HandleTypeDef * hmmc)
{
    ...
    errorstate =
        SDMMC_CmdSendCSD( hmmc->Instance,
                          (uint32_t) (hmmc->MmcCard.RelCardAdd << 16U) ) ;
    if ( errorstate != HAL_MMC_ERROR_NONE ) {
        return errorstate ;
    }
    else {
    ...
    /* Get the Card Class */
    hmmc->MmcCard.Class =
        (SDMMC_GetResponse(hmmc->Instance, SDMMC_RESP2) >> 20U) ;

    /* Select the Card */
    errorstate =
        SDMMC_CmdSelDesel( hmmc->Instance,
                           (uint32_t) ( ( (uint32_t) hmmc->MmcCard.RelCardAdd )
                                        << 16U ) ) ;
    if ( errorstate != HAL_MMC_ERROR_NONE ) {
        return errorstate ;
    }
    ...
    if ( HAL_MMC_GetCardExtCSD(hmmc, hmmc->Ext_CSD,
                               SDMMC_DATATIMEOUT) != HAL_OK ) {
        return hmmc->ErrorCode ;
    }
    ...
    FunctionalState state)
{
    ...
    if ( ( (hmmc->Instance->CLKCR & SDMMC_CLKCR_BUSSPEED) != 0U ) &&
         (state == DISABLE) ) {
        errorstate =
            MMC_PwrClassUpdate(hmmc,
                               (hmmc->Instance->CLKCR & SDMMC_CLKCR_WIDBUS),
                               SDMMC_SPEED_MODE_DEFAULT) ;
        if ( errorstate == HAL_MMC_ERROR_NONE ) {
            /* Index : 185 - Value : 0 */
    ...
    if ( errorstate == HAL_MMC_ERROR_NONE ) {
        ...
        do {
            errorstate =
                SDMMC_CmdSendStatus( hmmc->Instance,
                                     (uint32_t) ( ( (uint32_t) hmmc->MmcCard.
                                                    RelCardAdd ) << 16U ) ) ;
            if ( errorstate != HAL_MMC_ERROR_NONE ) {
                break ;
            }
                ...
                if ( state == DISABLE ) {
                    ...
                    if ( sdmmc_clk == 0U ) {
                        ...
                        if ( sdmmc_clk <= MMC_HIGH_SPEED_FREQ ) {
                            ...
                            Init.ClockDiv =
                                ( sdmmc_clk / (2U * MMC_HIGH_SPEED_FREQ) ) + 1U ;
                        }
                        (void) SDMMC_Init(hmmc->Instance, Init) ;
    ...
    uint32_t Speed)
{
    ...
    if ( (Wide == SDMMC_BUS_WIDE_8B) || (Wide == SDMMC_BUS_WIDE_4B) ) {
        ...
        if ( MMC_ReadExtCSD(hmmc, &power_class, 187,
                            SDMMC_DATATIMEOUT) != HAL_OK ) {                  /* Field POWER_CLASS [187] */
            errorstate = SDMMC_ERROR_GENERAL_UNKNOWN_ERR ;
        }
        else {
        ...
        if ( Speed == SDMMC_SPEED_MODE_DDR ) {
            ...
            supported_pwr_class =
                ( (hmmc->Ext_CSD[(MMC_EXT_CSD_PWR_CL_DDR_52_INDEX
                                  / 4)] >> MMC_EXT_CSD_PWR_CL_DDR_52_POS)
                  & 0x000000FFU ) ;
        }
        else if ( Speed == SDMMC_SPEED_MODE_HIGH ) {
            /* Field PWR_CL_52_xxx [200 or 202] */
            supported_pwr_class =
                ( (hmmc->Ext_CSD[(MMC_EXT_CSD_PWR_CL_52_INDEX
                                  / 4)] >> MMC_EXT_CSD_PWR_CL_52_POS)
                  & 0x000000FFU ) ;
        }
        else {
            /* Field PWR_CL_26_xxx [201 or 203] */
            supported_pwr_class =
                ( (hmmc->Ext_CSD[(MMC_EXT_CSD_PWR_CL_26_INDEX
                                  / 4)] >> MMC_EXT_CSD_PWR_CL_26_POS)
                  & 0x000000FFU ) ;
        }
        ...
        if ( errorstate == HAL_MMC_ERROR_NONE ) {
            ...
            if ( (power_class & 0x0FU) != (supported_pwr_class & 0x0FU) ) {
                ...
                errorstate =
                    SDMMC_CmdSwitch( hmmc->Instance,
                                     ( 0x03BB0000U
                                       | ( (supported_pwr_class & 0x0FU) << 8U ) ) ) ;
 ...
 * AGGIUNTE
 *
 * Uso il comando 23 in modo da poter accedere alla partizione RPMB
 *
 * Questo va fatto prima di invocare le funzioni seguenti
 * con la mmc_cmd23()
 *
 */
...
HAL_StatusTypeDef mmc_writeblocks_dma(
    MMC_HandleTypeDef * hmmc,
    uint8_t * pData,
    uint32_t BlockAdd,
    uint32_t NumberOfBlocks)
{
    ...
    if ( hmmc->State == HAL_MMC_STATE_READY ) {
        ...
        hmmc->Context = (MMC_CONTEXT_WRITE_MULTIPLE | MMC_CONTEXT_DMA) ;
...
HAL_StatusTypeDef mmc_readblocks_dma(
    MMC_HandleTypeDef * hmmc,
    uint8_t * pData,
    uint32_t BlockAdd,
    uint32_t NumberOfBlocks)
{
    ...
    if ( hmmc->State == HAL_MMC_STATE_READY ) {
        ...
        hmmc->Context = (MMC_CONTEXT_READ_MULTIPLE | MMC_CONTEXT_DMA) ;
