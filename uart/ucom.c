...
{
    if (attivo) {
        PIN_BAS(GPIOG, GPIO_PIN_12) ;
    }
    else {
        PIN_ALT(GPIOG, GPIO_PIN_12) ;
    }
}
...
static UCOM_CALLBACK pfCB = NULL ;
static void * cbARG = NULL ;

static UCOM_CALLBACK pfTx = NULL ;

#ifndef UCOM_DIM_FIFO
#   error MANCA UCOM_DIM_FIFO
#elif UCOM_DIM_FIFO == 0
#   error FIFO DI DIMENSIONE NULLA
#elif UCOM_DIM_FIFO > 0xFFFF
#   error FIFO TROPPO GRANDE
#else
#   define UART_DIM_DMA    UCOM_DIM_FIFO
...
bool UCOM_Iniz(UCOM_CALLBACK cb, void * arg)
{
    assert(cb) ;
    assert(NULL == pfCB) ;
...
bool UCOM_Tx(const void * tx, uint32_t dim)
{
    if (HAL_OK == HAL_UART_Transmit_DMA(&huart, (const uint8_t *) tx, dim)) {
    	stampa_roba("UCOM ->", tx, dim, 16) ;
...
bool UCOM_TxCB(const void * tx, uint32_t dim, UCOM_CALLBACK pf)
{
    assert(NULL == pfTx) ;

    pfTx = pf ;

    return UCOM_Tx(tx, dim) ;
}
...
uint32_t UCOM_Rx(void * rx, uint32_t dim)
{
    uint32_t letti = 0 ;
...
void UCOM_MSP_Iniz(void)
{
    ...
    */
    ...
    // Irq
    HAL_NVIC_SetPriority(USART6_IRQn, PERIF_IRQ, 0) ;
    HAL_NVIC_EnableIRQ(USART6_IRQn) ;

    HAL_NVIC_SetPriority(DMA2_Stream2_IRQn, PERIF_IRQ, 0) ;
    HAL_NVIC_EnableIRQ(DMA2_Stream2_IRQn) ;

    HAL_NVIC_SetPriority(DMA2_Stream6_IRQn, PERIF_IRQ, 0) ;
    HAL_NVIC_EnableIRQ(DMA2_Stream6_IRQn) ;
}

void UCOM_MSP_Fine(void)
{
    ...
    MZ_GPIO_DeInit(GPIOG, GPIO_PIN_14 | GPIO_PIN_9 | GPIO_PIN_13 | GPIO_PIN_12) ;
...
void UCOM_IdleCallback(void)
{
	pfCB(cbARG) ;
}

void UCOM_TxCpltCallback(void)
{
    if (pfTx) {
        pfTx(cbARG) ;
        pfTx = NULL ;
...
void UCOM_RxHalfCpltCallback(void)
{
    RTS(false) ;
#if 0
    // Non serve: scatta l'idle
...
void UCOM_ErrorCallback(void)
{
    ...
    if (hdma_tx.ErrorCode != HAL_DMA_ERROR_NONE) {
        ...
        if (HAL_DMA_ERROR_TE            & hdma_tx.ErrorCode) {
            DBG_PUTS("UCOM ERR TX: TE           ") ;
        }
        if (HAL_DMA_ERROR_FE            & hdma_tx.ErrorCode) {
            DBG_PUTS("UCOM ERR TX: FE           ") ;
        }
        if (HAL_DMA_ERROR_DME           & hdma_tx.ErrorCode) {
            DBG_PUTS("UCOM ERR TX: DME          ") ;
        }
        if (HAL_DMA_ERROR_TIMEOUT       & hdma_tx.ErrorCode) {
            DBG_PUTS("UCOM ERR TX: TIMEOUT      ") ;
        }
        if (HAL_DMA_ERROR_PARAM         & hdma_tx.ErrorCode) {
            DBG_PUTS("UCOM ERR TX: PARAM        ") ;
        }
        if (HAL_DMA_ERROR_NO_XFER       & hdma_tx.ErrorCode) {
            DBG_PUTS("UCOM ERR TX: NO_XFER      ") ;
        }
        if (HAL_DMA_ERROR_NOT_SUPPORTED & hdma_tx.ErrorCode) {
            DBG_PUTS("UCOM ERR TX: NOT_SUPPORTED") ;
        }
    }
#endif
    ...
    if (hdma_rx.ErrorCode != HAL_DMA_ERROR_NONE) {
        ...
        if ( HAL_OK != HAL_UART_Receive_DMA(&huart, dmacRx, UART_DIM_DMA) ) {
            BPOINT ;
        }
        else {
            RTS(true) ;
        ...
        if (HAL_DMA_ERROR_TE            & hdma_rx.ErrorCode) {
            DBG_PUTS("UCOM ERR RX: TE           ") ;
        }
        if (HAL_DMA_ERROR_FE            & hdma_rx.ErrorCode) {
            DBG_PUTS("UCOM ERR RX: FE           ") ;
        }
        if (HAL_DMA_ERROR_DME           & hdma_rx.ErrorCode) {
            DBG_PUTS("UCOM ERR RX: DME          ") ;
        }
        if (HAL_DMA_ERROR_TIMEOUT       & hdma_rx.ErrorCode) {
            DBG_PUTS("UCOM ERR RX: TIMEOUT      ") ;
        }
        if (HAL_DMA_ERROR_PARAM         & hdma_rx.ErrorCode) {
            DBG_PUTS("UCOM ERR RX: PARAM        ") ;
        }
        if (HAL_DMA_ERROR_NO_XFER       & hdma_rx.ErrorCode) {
            DBG_PUTS("UCOM ERR RX: NO_XFER      ") ;
        }
        if (HAL_DMA_ERROR_NOT_SUPPORTED & hdma_rx.ErrorCode) {
            DBG_PUTS("UCOM ERR RX: NOT_SUPPORTED") ;
        }
#endif
    }
}
