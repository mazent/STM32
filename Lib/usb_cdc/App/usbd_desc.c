...
#define USBD_VID                        0x32B3
#define USBD_LANGID_STRING              1033
#define USBD_MANUFACTURER_STRING        "Texa s.p.a."
...
{
    if ( speed == 0 ) {
        USBD_GetString( (uint8_t *) USBD_PRODUCT_STRING, USBD_StrDesc,
                        length ) ;
    }
    else {
        USBD_GetString( (uint8_t *) USBD_PRODUCT_STRING, USBD_StrDesc,
...
{
    if ( speed == 0 ) {
        USBD_GetString( (uint8_t *) USBD_INTERFACE_STRING, USBD_StrDesc,
                        length ) ;
    }
    else {
        USBD_GetString( (uint8_t *) USBD_INTERFACE_STRING, USBD_StrDesc,
...
/**
  ...
  * @retval Pointer to descriptor buffer
  */
uint8_t * USBD_HS_USR_BOSDescriptor(
    USBD_SpeedTypeDef speed,
    uint16_t * length)
{
    UNUSED(speed) ;
...
#if defined (USBD_SERIAL_0) && defined (USBD_SERIAL_1)
    // Evita di avere tante seriali
    deviceserial0 = USBD_SERIAL_0 ;
    deviceserial1 = USBD_SERIAL_1 ;
#else
