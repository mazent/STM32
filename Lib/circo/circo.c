...
bool CIRCO_ins(S_CIRCO * pC, const uint8_t * dati, uint16_t dim)
{
    ...
    do {
        ...
        if (1 == dim) {
            ...
            //     s      l
            // ****.......****
            memcpy_(pC->buf + scrivi, dati, dim) ;
            pC->tot += dim ;
        }
        else {
            //     l      s
            ...
            memcpy_(pC->buf + scrivi, dati, CODA) ;
            pC->tot += CODA ;

            dim -= CODA ;
            if (dim) {
                memcpy_(pC->buf, dati + CODA, dim) ;
                pC->tot += dim ;
            }
        }
    } while (false) ;
        ...
        while (dim) {
            ...
            if (1 == DIM) {
                *dati = pC->buf[pC->leggi] ;
                ...
                memcpy_(dati, pC->buf + pC->leggi, DIM) ;
            }
#ifndef NDEBUG
            memset_(pC->buf + pC->leggi, RIEMPITIVO_DI_DBG, DIM) ;
#endif
            dati += DIM ;
            letti += DIM ;
            pC->tot -= DIM ;
...
uint16_t CIRCO_est2(S_CIRCO * pC, uint8_t finoa, uint8_t * dati, uint16_t dim)
{
    ...
    do {
        ...
        if (fa) {
            ...
            memcpy_(dati, l, letti) ;
#ifndef NDEBUG
            memset_(l, RIEMPITIVO_DI_DBG, letti + 1) ;
#endif
            pC->tot -= letti + 1 ;
            ...
            if (fa) {
                ...
                memcpy_(dati, l, DIM) ;
#ifndef NDEBUG
                memset_(l, RIEMPITIVO_DI_DBG, DIM) ;
#endif
                dati += DIM ;
                letti += DIM ;
                ...
                letti += diff ;
                memcpy_(dati, pC->buf, diff) ;
#ifndef NDEBUG
                memset_(pC->buf, RIEMPITIVO_DI_DBG, diff + 1) ;
#endif
